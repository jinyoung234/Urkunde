...
import { Correct, Incorrect, ModifyPageTitle } from "src/components/commons/Commons";
import { Button } from "src/components/commons/PrimaryButton";
import useAnswerCheck from "src/hooks/useAnswerCheck"
import { answerButtonState, isCorrect } from "src/utils/storage";
    ...
    return(
        ...
        {isWriteAnswer && 
            <ModifyPageTitle style={{marginTop: '15px', display:'flex', flexDirection:'column'}}>
                {isCorrectAnswer ? <Correct>정답입니다 :)</Correct> : <Incorrect>틀렸습니다 :)</Incorrect>}
                {answer}
            </ModifyPageTitle>
        }
        </>
